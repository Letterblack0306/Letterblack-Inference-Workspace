...
class ClineProviderHandler(GatewayCapabilityHandler):
    """Adds optional, evidence-guarded Cline CLI routes without changing GGUF routing."""
    ...
    def do_POST(self) -> None:
        ...
        if parts[-1] == "profiles":
            ...
            def add_profile(state: dict[str, Any]):
                ...
                base.add_log(
                    ...
                    "info",
                    "cline",
                    "Authorized Cline profile registered.",
        ...
        try:
            result = cline_complete(provider.get("profiles", []), prompt, cwd=str(cwd))
        except ClineProviderError as exc:
            self._fail(exc.code, str(exc), 503, exc.details)
            return
        base.STORE.mutate(
            lambda current: base.add_log(
                current,
                "info",
                "cline",
                ...
                evidenceGuard=result.get("evidenceGuard"),
            )
        )
        self._ok(result)
                ...
                for index, item in enumerate(provider["profiles"]):
                    if item["id"] == profile_id:
                        ...
                        base.add_log(
                            ...
                            "info",
                            "cline",
                            "Authorized Cline profile updated.",
    ...
    def do_DELETE(self) -> None:
        ...
        if parts[:4] == ["api", "v1", "providers", "cline"] and len(parts) == 5:
            ...
            def remove_profile(state: dict[str, Any]):
                ...
                provider["profiles"] = [
                    item for item in provider["profiles"] if item["id"] != profile_id
                ]
                ...
                base.add_log(
                    ...
                    "warning",
                    "cline",
                    "Authorized Cline profile removed.",
                    profileId=profile_id,
                )
                return True
...
def main() -> None:
    base.CAPABILITIES["contractVersion"] = "6.2.0"
    base.CAPABILITIES["features"]["clineProvider"] = "authorized-profiles-evidence-guarded"
    parser = argparse.ArgumentParser(
        description="Letterblack Inference Workspace with optional evidence-guarded Cline CLI provider"
    )
    ...
    print(f"Letterblack Phase 6.2 serving http://{args.host}:{args.port}")
    print("Local GGUF routing unchanged; optional evidence-guarded Cline provider enabled.")
    try:
